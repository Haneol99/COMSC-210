...
public:
    ...
    void addHead(double, const string &);
    void outputting() const;
    double randomRating();
    void fileComment(ifstream &, int , const string &);
};
...
void Movie::addHead(double r, const string &c)
{
...
double Movie::randomRating()
{
    double randomRating = (static_cast<double>(rand()) / RAND_MAX * 4.0) + 1.0;
    return randomRating;
}

void Movie::fileComment(ifstream &f, int j, const string &file)
    {
        string comment;
        for (int i = 0; i < j; i++)
        {
